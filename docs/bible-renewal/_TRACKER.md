# Bible 페이지 리뉴얼 - 태스크 추적

> **시작일**: 2026-01-06
> **상태**: 진행 중
> **기획 문서**: [bible-page-renewal-plan.md](../bible-page-renewal-plan.md)

---

## 진행 규칙

1. **순차 진행**: 태스크는 순서대로 진행
2. **빌드 테스트**: 각 태스크 완료 후 `npm run build` 성공 확인
3. **기능 테스트**: Chrome DevTools MCP로 개발서버에서 테스트
4. **커밋 발행**: 테스트 성공 후 커밋
5. **완료 처리**: 태스크 문서에 완료 표시 및 이 문서 업데이트
6. **정리**: 모든 태스크 완료 시 이 폴더 삭제

---

## 전체 진행 현황

| Phase | 태스크 | 진행률 |
|-------|--------|--------|
| Phase 1 | 기반 작업 | 0/3 |
| Phase 2 | 핵심 기능 | 0/4 |
| Phase 3 | 부가 기능 | 0/3 |
| Phase 4 | 설정 및 마무리 | 0/4 |
| **Total** | | **0/14** |

---

## Phase 1: 기반 작업

| # | 태스크 | 상태 | 커밋 |
|---|--------|------|------|
| 1-1 | [라우트 구조 및 리다이렉트](./phase-1/task-1-1-routes.md) | 🔄 진행 중 | - |
| 1-2 | [reading.vue 컴포넌트 분리](./phase-1/task-1-2-component-split.md) | ⬜ 대기 | - |
| 1-3 | [/bible 페이지 기본 구조](./phase-1/task-1-3-bible-page.md) | ⬜ 대기 | - |

---

## Phase 2: 핵심 기능

| # | 태스크 | 상태 | 커밋 |
|---|--------|------|------|
| 2-1 | [성경 뷰어 컴포넌트](./phase-2/task-2-1-bible-viewer.md) | ⬜ 대기 | - |
| 2-2 | [읽기모드 구현](./phase-2/task-2-2-reading-mode.md) | ⬜ 대기 | - |
| 2-3 | [통독모드 구현](./phase-2/task-2-3-tongdok-mode.md) | ⬜ 대기 | - |
| 2-4 | [마지막 읽기 위치](./phase-2/task-2-4-reading-position.md) | ⬜ 대기 | - |

---

## Phase 3: 부가 기능

| # | 태스크 | 상태 | 커밋 |
|---|--------|------|------|
| 3-1 | [북마크 기능](./phase-3/task-3-1-bookmark.md) | ⬜ 대기 | - |
| 3-2 | [묵상노트 기능](./phase-3/task-3-2-notes.md) | ⬜ 대기 | - |
| 3-3 | [하이라이트 기능](./phase-3/task-3-3-highlight.md) | ⬜ 대기 | - |

---

## Phase 4: 설정 및 마무리

| # | 태스크 | 상태 | 커밋 |
|---|--------|------|------|
| 4-1 | [읽기 설정 페이지](./phase-4/task-4-1-settings.md) | ⬜ 대기 | - |
| 4-2 | [읽기 기록/통계 페이지](./phase-4/task-4-2-history.md) | ⬜ 대기 | - |
| 4-3 | [기본 진입점 설정](./phase-4/task-4-3-entry-point.md) | ⬜ 대기 | - |
| 4-4 | [최종 테스트 및 정리](./phase-4/task-4-4-final.md) | ⬜ 대기 | - |

---

## 커밋 이력

| 날짜 | 태스크 | 커밋 해시 | 메시지 |
|------|--------|----------|--------|
| - | - | - | - |

---

## 상태 범례

- ⬜ 대기
- 🔄 진행 중
- ✅ 완료
- ❌ 블로커
