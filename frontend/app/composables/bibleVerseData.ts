 ...
 * @param bookId 책 ID
 * @param chapter 장 번호 (1-based)
 * @param verse 절 번호 (1-based), null이면 장만 검증
 * @returns 유효 여부
 */
export function isValidReference(bookId: string, chapter: number, verse: number | null): boolean {
  const verses = VERSE_COUNTS[bookId];
  if (!verses) return false;
  
  // 장 검증
  if (chapter < 1 || chapter > verses.length) return false;
  
  // 절 검증 (절이 지정된 경우)
  if (verse !== null) {
    const maxVerse = verses[chapter - 1];
    if (!maxVerse || verse < 1 || verse > maxVerse) return false;
  }
